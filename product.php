...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			//----lhj----2023年5月17日
			// 获取产品 URL
			$lhj_product_url = htmlspecialchars_decode( $this->url->link('product/product', 'product_id=' . $this->request->get['product_id']) );
			
			
			// 生成二维码并将其输出到输出缓冲区
			ob_start();
			QRcode::png( $lhj_product_url, null, QR_ECLEVEL_H, 4);
			$lhj_product_img = ob_get_contents();
			ob_end_clean();

			// 生成二维码的数据 URL
			$data['lhj_product_url'] = 'data:image/png;base64,' . base64_encode( $lhj_product_img );
			//---lhj
